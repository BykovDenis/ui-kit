...
const MultiSelect: React.FunctionComponent<PropsWithChildren<TMultiSelect>> = (props: TMultiSelect) => {
  ...
  const color: string = props.disabled ? cssVariables.baseFontColorOpacity05 : cssVariables.baseFontColor;
  const outlinedColor: string = cssVariables.mainOutlinedColor;
  ...
  const fontSize: number | string = props.fontSize ?? thcssVariableseme.baseFontSize;
  ...
  return (
    <MultiSelectContainerStyled
      ...
      borderColorFocused={cssVariables.primaryLightColor}
      borderColorHovered={cssVariables.mainOutlinedHoverColor}
      onClick={onBtnElementsClickExpand}
      ref={btnMultiSelect}
    >
      {props?.label && (
        <LabelContainer backgroundColor={cssVariables.backgroundColor}>
          <Label
      ...
      <FormControl>
        <MultiSelectStyled
          ...
          borderColor={cssVariables.baseFontColorOpacity05}
        >
          {arrElementNames?.map((columnNameElement: string, index: number) => (
            <FormControl
              key={`${index}-button`}
              width="initial"
              outline={`1px solid ${cssVariables.baseFontColorOpacity05}`}
              borderRadius={5}
              padding="1px"
              margin="0 3px 0 0"
              backgroundColor={cssVariables.primaryLightColor}
            >
              <Label
                fontSize={pixelsMeasureToNumber(fontSize) - 2}
              ...
              <ButtonStyled id={`${index}-button`} onClick={onColumnNameRemove} data-name={columnNameElement}>
                <CrossIcon color={cssVariables.baseFontColor} />
              </ButtonStyled>
            </FormControl>
          ))}
        </MultiSelectStyled>
        ...
        <ToggleContainer data-name="toggle-container" ref={btnToggleContainer}>
          <FormControl>
            <Button
              ...
              backgroundColor={cssVariables.primaryLightColor}
            >
              Select all <CirclePlusIcon color={cssVariables.baseFontColor} />
            </Button>
            <Button
              padding="2px"
              width="50%"
              ...
              backgroundColor={cssVariables.primaryLightColor}
            >
              Unselect all <CircleCrossIcon color={cssVariables.baseFontColor} />
            </Button>
          </FormControl>
          <Input
            name="search-values"
                ...
                >
                  <Label backgroundColor="transparent" data-value={columnNameElement}>
                    ...
                    <FormControl position="absolute" right={5} width="initial" data-value={columnNameElement}>
                      <CirclePlusIcon color={cssVariables.baseFontColor} />
                    </FormControl>
                  </Label>
                </ListItem>
              ))}
